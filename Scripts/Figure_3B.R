...
#===============================================================================
#****************************************************************************
#' Uses ComplexHeatmap and a set of matrices to plot Figure 3B
#****************************************************************************
################################################################################
...
#'  in WCDT cohort. I also excluded Linear amplification in this analysis
#===============================================================================

plot_genes_AA_wcdt_ht <- function(genes_to_include, include_oncoprint = TRUE, row_split,complex_events,
    ...
    if (include_clinical_info) {
      ...
      event_col_fun <- c("ecDNA"="#EB8D71", "BFB"="#009392","Complex"= "#9CCB86", "Unknown" = "#EBCE8D")
        ...
        lgd_wgd = Legend(title = "WG Doubling", labels = names(wgd_col_fun), legend_gp = gpar(fill = unname(wgd_col_fun)), border = "black")
        ...
        lgd_tp53 = Legend(title = "TP53", labels = names(TP53_col_fun), legend_gp = gpar(fill = unname(TP53_col_fun)), border = "black")
        lgd_pten = Legend(title = "PTEN", labels = names(PTEN_col_fun), legend_gp = gpar(fill = unname(PTEN_col_fun)), border = "black")
...
#===============================================================================
# To use the function plot_genes_AA_wcdt_ht, split_ecDNA_wcdt_gene_list, id_order_wcdt,
# wcdt_events_mod, SO_WCDT_summary_sub, and matrix_AA_gene_wcdt_freq_sub_new_mod 
# are necessary!

plot_genes_AA_wcdt_ht(genes_to_include,  include_oncoprint = T, complex_events = wcdt_events_mod[,3:6],
                      sample_full = SO_WCDT_summary_sub, include_clinical_info = T, matrix_oncoP = matrix_AA_gene_wcdt_freq_sub_new_mod,
                      id_order = id_order_wcdt, save = T, add_line_in_annotation = F, row_split = split_ecDNA_wcdt_gene_list[,1],
                      fn_output_plot = "Figure_3B.pdf", width = 20, height = 12)
